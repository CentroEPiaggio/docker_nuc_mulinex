...
from pi3hat_moteus_int_msgs.msg import (
    JointsStates,
    OmniMulinexCommand,
)
...
class JumpingTest(Node):
    def __init__(
        ...
        pre_time=5,  # [s]
        jnt_names=['LF_WHEEL', 'RF_WHEEL', 'RH_WHEEL', 'LH_WHEEL'],  # QUELLI DELLA CONFIG
        test_name='prova_jump',
        n_joint=4,  # numero di giunti
        try_test=100,  # [salti da eseguire]
        try_start=1,
        ratio=9,
        delta=10000000000000000,  # [rad]
        Vel_linear=[2.0],  # [m/s] velocità lineare
        Vel_angolar=[2.0],  # [rad/s] velocità angolare
        Vel_start=[0.0],
        height_rate=[0.0],
        # offset = [math.pi,0.0],
        offset=[0.0, 0.0],
        ...
        super().__init__(
            node_name
        )  # super chiama il costruttore della calsse Node e dà il nome ( node_name = Jumping_node)
        ...
        self.pub_command_vel = self.create_publisher(
            OmniMulinexCommand,  # il nodo pubblica messaggi di tipo OmniMulinexCommand, sul topic chiamato joint_controller/command e la dimensione della coda è 10
            'joint_controller/pi3hat_vel_controller/command',
            10,
        )

        self.pub_count = self.create_publisher(
            Counter,  # il nodo pubblica messaggi di tipo JointsCommand, sul topic chiamato joint_controller/command e la dimensione della coda è 10
            'Counter',
            10,
        )
        ...
        # finire did defnire i valori di vel in ogni fase e controllare messagig fdando campo vel[], qiua dare vel di ogno cosa in ogni campo
    ...
    def homing_callback(self):
        ...
        if (self.t < self.t_pre_test) and (self.state_moviment >= 0):
            ...
            self.publish_msg(
                ...
                v_y_init=self.v_y[0],
                v_y_fin=self.v_y[1],
                v_omega_init=self.omega[0],
                v_omega_fin=self.omega[1],
                time_ini=0.0,
                time_fin=self.t_pre_test,
                phase_test=self.phase_test,
                phase_moviment=self.state_moviment,
            )
            ...
            self.timer = self.create_timer(
                self.period, self.timer_callback_iteration
            )  ## starting multi jum  ## starting the real jump   se commento questo non va alla fase di salto
    ...
    def timer_callback_moviment(self):
        ...
        elif (self.counter_move > 0) and (self.counter_move <= 1):
    ...
    def publish_msg(
        ...
        self.v_x = self.traj_linear_ik.linear_trajectory(
            pos_init=v_x_init, pos_fin=v_x_fin, time_ini=time_ini, time_fin=time_fin, time=self.t
        )

        self.v_y = self.traj_linear_ik.linear_trajectory(
            pos_init=v_y_init, pos_fin=v_y_fin, time_ini=time_ini, time_fin=time_fin, time=self.t
        )

        self.omega = self.traj_linear_ik.linear_trajectory(
            pos_init=omega_init,
            pos_fin=omega_fin,
            time_ini=time_ini,
            time_fin=time_fin,
        ...
        if not (math.isnan(self.v_x) or math.isnan(self.v_y) or math.isnan(self.omega)):
...
def main(args=None):
    ...
    node = JumpingTest(
        test_name=name_exp, q_home_1=[-0.7, 2.84], period=1 / 450
